...
# 음악 가져와서 재생==============================================================================
music_file = "songs/PADO - Great Means Not Good - 06 Cloud Castle.mp3"
pygame.mixer.music.load(music_file)

# 음악 파일 길이 측정
audio = MP3(music_file)
music_length = audio.info.length - 6 # 음악 안나오는 시간 6초 정도 뺸 값
...
def rating(n):
    global combo, miss_anim, last_combo, combo_effect, combo_effect2, combo_time, rate, current_health, game_over, game_end, music_length, worst_count, miss_count, bad_count, good_count, great_count, perpect_count, max_combo,Score
...
# 생명력 조정
    if rate == "MISS":
        ...
        current_health -= 0  # worst 판정 시 생명력 감소 추가
    elif rate == "BAD": 
        Score += 30          # 스코어 30점
        bad_count += 1       # bad 카운트
        current_health -= 0  # BAD 판정 시 생명력 감소
    elif rate == "GOOD":
        Score += 50          # 스코어 50점
        good_count += 1      # good 카운트
        current_health += 0  # GOOD 판정 시 생명력 증가
    elif rate == "GREAT":
        Score += 70          # 스코어 70점
        great_count += 1     # great 카운트
        current_health += 0  # GREAT 판정 시 생명력 증가
    elif rate == "PERPECT":
        Score += 100         # 스코어 100점
        perpect_count += 1   # perpect 카운트
        current_health += 0  # PERFECT 판정 시 생명력 증가
...
#게임오버=====================================================================================================================================
    if game_over:
        ...
        while input_active:
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:       #엔터키를 누르면 탈출
                        input_active = False
            ...
            score_text = music_font.render("2번 곡", True , (255, 255, 255))
            ...
            screen.blit(score_text, (180, 600))
            score_text = score_font.render(f"{int(bad_count)}", True , (255, 255, 255))
            screen.blit(score_text, (180, 700))

            score_text = score_font.render("WORST", True, (255, 255, 255))
            screen.blit(score_text, (340, 600))
            score_text = score_font.render(f"{int(worst_count)}", True , (255, 255, 255))
            screen.blit(score_text, (340, 700))

            score_text = score_font.render("MISS", True, (255, 255, 255))
            screen.blit(score_text, (590, 600))
            score_text = score_font.render(f"{int(miss_count)}", True , (255, 255, 255))
            screen.blit(score_text, (590, 700))

            score_text = score_font.render("Score", True, (255, 255, 255))
            screen.blit(score_text, (880, 600))
            score_text = score_font.render(f"{int(Score)}", True , (255, 255, 255))
            screen.blit(score_text, (880, 700))
        
            pygame.draw.rect(screen, (255, 255, 255), (600, 800, 400, 50), 2)
            name_surface = input_font.render(name, True, (255, 255, 255))
            screen.blit(name_surface, (610, 810))
    
            input_text = music_font.render("이름을 입력하세요: ", True, (255, 255, 255))
            screen.blit(input_text, (600, 750))
    
            pygame.display.flip()


        # 이름 입력이 완료된 후 처리
        update_ranking(name, Score)
        score_text = score_font.render("Ranking", True, (255, 255, 255))
        screen.blit(score_text, (1200, 200))
        y_offset = 300
        for rank in rankList:
            score_text = score_font.render(f"{rank.name}: {rank.score}", True, (255, 255, 255))
            screen.blit(score_text, (1200, y_offset))
